...
class Evaluacion_calidad_model extends CI_Model {
    ...
    public function get_calidad_global_secciones_rraa_periodo($cve_periodo) {
        $sql = "select dci.cve_periodo, dci.cve_seccion, sum(valor_max_sna) as sum_valor_max_sna, sum(dci.valor) as sum_valor, ((sum(valor) * 100) / sum(valor_max_sna)) as indicador, (case when ((sum(valor) * 100) / sum(valor_max_sna)) < 25 then 0 when ((sum(valor) * 100) / sum(valor_max_sna)) < 50 then 1 when ((sum(valor) * 100) / sum(valor_max_sna)) < 70 then 2 when ((sum(valor) * 100) / sum(valor_max_sna)) < 80 then 3 when ((sum(valor) * 100) / sum(valor_max_sna)) < 90 then 4 when ((sum(valor) * 100) / sum(valor_max_sna)) >= 90 then 5 end) as indicador_calidad_seccion, count(*) filter (where valor=0) as sin_info, count(*) filter (where valor=1) as alertas from datos_calidad_indicadores dci where dci.cve_seccion in (2,3,4) and dci.cve_periodo = ? group by dci.cve_periodo, dci.cve_seccion ";
