...
class Cuestionarios_contestados_model extends CI_Model {
    ...
    public function get_semaforos_cuestionario_contestado($cve_cuestionario_contestado) {
        $sql = "select ss.cve_subseccion, ((select count(*) from preguntas p left join subpreguntas sp on p.cve_pregunta = sp.cve_pregunta where p.cve_subseccion = ss.cve_subseccion and sp.cve_pregunta IS NULL) + (select count(*) from preguntas p inner join subpreguntas sp on p.cve_pregunta = sp.cve_pregunta where p.cve_subseccion = ss.cve_subseccion)) as preguntas, (select count(*) as respuestas from respuestas r left join preguntas p on r.cve_pregunta = p.cve_pregunta where valor <> '' and r.cve_cuestionario_contestado = cc.cve_cuestionario_contestado and p.cve_subseccion = ss.cve_subseccion ) as respuestas from subsecciones ss left join secciones s on ss.cve_seccion = s.cve_seccion left join cuestionarios c on s.cve_cuestionario = c.cve_cuestionario left join cuestionarios_contestados cc on c.cve_cuestionario = cc.cve_cuestionario where cc.cve_cuestionario_contestado = ? order by ss.cve_subseccion ;";
        $query = $this->db->query($sql, array($cve_cuestionario_contestado));
        return $query->result_array();
    }
